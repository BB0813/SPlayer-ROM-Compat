  ...
  getters: {
    ...
    songCover(state): string {
      return state.playSong.path
        ? normalizeImageUrl(state.playSong.cover)
        : normalizeImageUrl(state.playSong.coverSize?.s || state.playSong.cover);
    },
    // 私人FM播放歌曲
  ...
  actions: {
    ...
    getSongCover(size: "s" | "m" | "l" | "xl" | "cover" = "s") {
      return this.playSong.path
        ? normalizeImageUrl(this.playSong.cover)
        : size === "cover"
          ? normalizeImageUrl(this.playSong.cover)
          : normalizeImageUrl(this.playSong.coverSize?.[size] || this.playSong.cover);
    },
  },
  // 持久化
  persist: {
